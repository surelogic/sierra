...
public class ManageServerServiceImpl extends SierraServiceServlet implements ManageServerService {
  ...
  public ServerInfo deploySchema() {
    return performAdmin(false, new UserTransaction<ServerInfo>() {
      ...
      public ServerInfo perform(final Connection conn, final Server server, final User user) throws SQLException {
        try {
          server.updateSchema();
    ...
    if (info.isValid()) {
      return performAdmin(false, new UserTransaction<ServerInfo>() {
        ...
        public ServerInfo perform(final Connection conn, final Server server, final User user) throws SQLException {
          ...
          final Integer port = portStr == null ? null : Integer.valueOf(portStr);
          server.setNotification(
              new Notification(info.getHost(), port, info.getUser(), info.getPass(), info.getAdminEmail(), info.getServerEmail()));
          return readServerInfo(server);
        }
      });
    } else {
  ...
  public Status testAdminEmail() {
    try {
      performAdmin(true, new UserTransaction<Void>() {
        ...
        public Void perform(final Connection conn, final Server server, final User user) throws Exception {
          final String name = server.getName();
          server.notifyAdmin(String.format("Sierra Team Server '%s' Email Test", name),
              String.format("If you received this email, server exception notification is configured properly for '%s'", name));
          return null;
        }
      });
      ...
      return new Status(false, String.format(
          "An error occurred while trying to send a test message.  See the server log for more details: %s", e.getMessage()));
    }
  }
  ...
  ServerInfo readServerInfo(final Server server) throws SQLException {
    ...
    try {
      ...
      if (n != null) {
        ...
        info.setEmail(new EmailInfo(n.getHost(), port == null ? null : port.toString(), n.getUser(), n.getPassword(),
      ...
      info.setEmail(new EmailInfo(error, error, error, error, error, error));
    }
    return info;
  }
}
