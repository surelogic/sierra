...
public final class SettingsServiceImpl extends SierraServiceServlet implements
                ...
                new UserQuery<List<Project>>() {
                    public List<Project> perform(final Query q, final Server s,
                        ...
                        for (final ProjectDO projectDO : projects
                            ...
                            prj.setScanFilter(ServiceUtil.getFilter(
                                    filters.getScanFilter(sfUuid), types, cats));
                            result.add(prj);
                        }
                        return result;
                    }
                            ...
                            q.prepared("FilterSets.scanFiltersUsing",
                                    new RowHandler<Void>() {
                                        public Void handle(final Row r) {
                                            set.getScanFiltersUsing()
                                                    .add(new Category.ScanFilterInfo(
                                                            r.nextString(),
                                                            r.nextString()));
                                            return null;
                                        }
    ...
    FindingType getType(final FindingTypeDO type, final Query q) {
        ...
        for (final ArtifactTypeDO artDO : type.getArtifactTypes()) {
            final ArtifactTypeInfo newInfo = new ArtifactTypeInfo(
                    artDO.getTool(), artDO.getMnemonic());
    ...
    public List<PortalServerLocation> listServerLocations() {
        ...
        for (final ConnectedServer cs : ConnectionFactory
            ...
            final PortalServerLocation s = new PortalServerLocation(
                    cs.getName(), cs.getUuid(),
                    PortalServerLocation.Protocol.fromValue(l.getProtocol()),
                    l.getHost(), l.getPort(), l.getContextPath(), l.getUser(),
                ...
                new DBQuery<Status>() {
                    ...
                    public Status perform(final Query q) {
                        ...
                        return Status.success((server == null ? "Server"
                                : server.getName()) + " deleted.");
                    }
                });
    }

    public Status saveServerLocation(final PortalServerLocation loc) {
        final ServerLocation l = new ServerLocation(loc.getHost(),
                loc.getProtocol() == Protocol.HTTPS, loc.getPort(),
                loc.getContext(), loc.getUser(), loc.getPass(), true, true);
        ConnectedServer cs;
        if (loc.getUuid() == null) {
