...
public class ServerLocation {
   ...
   * @param user
   *          a saved user name to use with this server, or {@code null} is the
   *          user name is not saved.
   * @param pass
   *          a saved password to use with this server, or {@code null} is the
   *          password is not saved.
   * @param savePassword
   *          {@code true} if the password should persist longer than this
   *          session, {@code false} if not.
   * @param autoSync
   *          {@code true} if the the client should automatically synchronize
   *          with this server, {@code false} if not.
   * 
   * @throws IllegalArgumentException
   *           if <tt>host</tt> or <tt>contextPath</tt> are {@code null}.
   */
  public ServerLocation(final String host, final boolean secure, final int port, final String contextPath, final String user,
      final String pass, final boolean savePassword, final boolean autoSync) {
    if (host == null) {
   ...
   * <p>
   * This constructor is useful in the user interface at times, but because this
   * is an immutable class is rarely useful in other contexts.
   */
  public ServerLocation() {
    this(ServerLocation.DEFAULT_HOST, false, ServerLocation.DEFAULT_PORT, ServerLocation.DEFAULT_CONTEXT_PATH, null, null, false,
        false);
  }
   ...
   */
  public ServerLocation changeAuthorization(final String user, final String pass, final boolean savePassword) {
    return new ServerLocation(f_host, f_secure, f_port, f_contextPath, user, pass, savePassword, f_autoSync);
  }
  ...
  public ServerLocation changeAutoSync(final boolean autoSync) {
    return new ServerLocation(f_host, f_secure, f_port, f_contextPath, f_user, f_pass, f_savePassword, autoSync);
  }
  ...
  /**
   * Gets the protocol, either <tt>http</tt> or <tt>https</tt>, to this server.
   * 
   ...
   * 
   * @return the saved user name, or {@code null} if the user name is not saved.
   */
  ...
  /**
   * Gets the saved password, or the empty string if the password is not saved.
   * 
   ...
   * services should be hosted under the context root "/sierra", and have a name
   * that is valid w/in a URL string.
   * 
   * @param serviceName
   *          a service name.
  ...
  public URL createServiceURL(final String serviceName) {
    ...
    try {
      return new URL((f_secure ? "https://" : "http://") + host + f_contextPath + "services/" + serviceName);
  ...
  public URL createHomeURL() {
    final String host = getHost() + ":" + getPort();
    try {
      return new URL((f_secure ? "https://" : "http://") + host + f_contextPath);
    } catch (final MalformedURLException e) {
      throw new IllegalStateException(e);
    }
  }
  ...
  public String toString() {
    ...
    b.append(getHost()).append(":").append(getPort()).append(getContextPath());
  ...
  public int hashCode() {
    ...
    result = prime * result + ((f_contextPath == null) ? 0 : f_contextPath.hashCode());
