...
public class ClientSettingsManager extends SettingsManager {
	...
	private ClientSettingsManager(Connection conn) throws SQLException {
		...
		findingTypeFilterMapper = new BaseMapper(
				...
				"SELECT DELTA,IMPORTANCE,FILTERED FROM PROJECT_FILTERS WHERE PROJECT_ID = ? AND FINDING_TYPE_ID = ?",
				"DELETE FROM PROJECT_FILTERS WHERE PROJECT_ID = ? AND FINDING_TYPE_ID = ?");
		getFiltersByProjectId = conn
				.prepareStatement("SELECT FT.UID,F.DELTA,F.IMPORTANCE,F.FILTERED FROM PROJECT_FILTERS F, FINDING_TYPE FT WHERE F.PROJECT_ID = ? AND FT.ID = F.FINDING_TYPE_ID");
	}
