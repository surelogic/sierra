...
public abstract class AbstractScan<T extends IJavaElement> {
	...
	protected StringBuilder computeLabel(final Collection<String> names) {
		final StringBuilder sb = new StringBuilder(isRescan ? "Re-scanning "
				: "Scanning ");
		/*
					...
					public IStatus runInWorkspace(IProgressMonitor monitor)
						...
						try {
							...
							if (saved & built & compiled) {
								...
								BalloonUtility
										.showMessage(
												"Something isn't compiled",
												"Sierra cannot scan if your code isn't fully compiled");
							} else if (!compiled) {
								BalloonUtility
										.showMessage(
												"Something doesn't compile in "
														+ computeLabel(erroneous),
												"Sierra cannot scan if your code does not compile");
							} else {
								// Scan not run, because of modified editors
								BalloonUtility
										.showMessage("Modified editors",
	...
	protected final boolean setupConfigs(final List<Config> configs) {
		...
		UIJob job = new UIJob("Put up test dialog") {
			...
			public IStatus runInUIThread(IProgressMonitor monitor) {
				...
				final ScanTestCodeSelectionDialog dialog = new ScanTestCodeSelectionDialog(
						shell, configs);
