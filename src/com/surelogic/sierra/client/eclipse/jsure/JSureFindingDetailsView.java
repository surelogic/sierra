...
import com.surelogic.sierra.client.eclipse.views.AbstractSierraMultiPageView;
import com.surelogic.sierra.jdbc.finding.FindingDetail;

public class JSureFindingDetailsView extends
		AbstractSierraMultiPageView<JSureFindingDetailsMediator> {
	...
	@Override
	protected JSureFindingDetailsMediator createMorePartControls(
			Composite[] parents) {
		...
		for (Composite parent : parents) {
			 ...
			 * labels[i].setText("No finding yet"); labels[i].setLayoutData(new
			 * GridData(SWT.LEFT, SWT.TOP, true, false));
			 */
			viewers[i] = new TreeViewer(parent, SWT.SINGLE);
			viewers[i].getTree().setLayoutData(
		...
		return new JSureFindingDetailsMediator(this, parents, labels, viewers); // FIX
																				// tree);
	}
	...
	public static void findingSelected(FindingDetail detail, boolean moveFocus) {
		...
		try {
			if (moveFocus) {
				view = (JSureFindingDetailsView) EclipseUIUtility.showView(ID);
			} else {
				view = (JSureFindingDetailsView) EclipseUIUtility.showView(ID,
