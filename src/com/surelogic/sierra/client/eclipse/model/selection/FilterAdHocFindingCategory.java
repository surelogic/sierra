...
public final class FilterAdHocFindingCategory extends Filter implements
	...
	/*
	 * @Override public Image getImageFor(String value) { return
	 * SLImages.getImage(CommonImages.IMG_CATEGORY); }
	 */
		...
		synchronized (buglink) {
			...
			while (it.hasNext()) {
				...
				for (String findingType : buglink.getFindingTypes(cat)) {
					final FindingTypeDO def = buglink
	...
	protected Iterable<String> getMappedPorousValues() {
		...
		synchronized (BuglinkData.getInstance()) {
			for (String value : f_porousValues) {
				Collection<String> types = BuglinkData.getInstance()
						.getFindingTypes(value);
				if (types != null) {
					porous.addAll(types);
				}
			}
			 ...
			 * for(String value : porous) { String typeName =
