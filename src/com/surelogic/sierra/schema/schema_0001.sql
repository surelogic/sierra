...
CREATE VIEW FINDINGS_OVERVIEW
   (PROJECT,PACKAGE_NAME,CLASS_NAME,LOC,SUMMARY,IS_READ,IMPORTANCE_CODE,IMPORTANCE,NAME,FINDING_ID)
AS SELECT
...
FROM
   ...
   FINDING F,
   ARTIFACT_TYPE_FINDING_TYPE_RELTN ATFTR,
   FINDING_TYPE FT,
   SOURCE_LOCATION S,
   COMPILATION_UNIT CU
WHERE
   ...
   ATFTR.ARTIFACT_TYPE_ID = A.ARTIFACT_TYPE_ID AND
   FT.ID = ATFTR.FINDING_TYPE_ID;
