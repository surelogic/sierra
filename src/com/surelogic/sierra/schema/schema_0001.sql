...
CREATE VIEW FINDING_ARTIFACT_COUNT
   (SCAN,FINDING,COUNT)
AS
   SELECT S.ID,AFR.FINDING_ID,COUNT(AFR.ARTIFACT_ID)
   FROM SCAN S,ARTIFACT A, ARTIFACT_FINDING_RELTN AFR
   WHERE
      A.SCAN_ID = S.ID AND
      AFR.ARTIFACT_ID = A.ID
   GROUP BY S.ID,AFR.FINDING_ID;
