...
public class CPD5_2_3ToolInfoGenerator {
  public static void generateTool(Connection conn) {
    
    try {
      ArtifactTypeBuilder rule = ToolBuilder.getBuilder(conn).build("CPD", "5.2.3");
      ...
      throw new IllegalStateException("Could not build CPD tool info for v.5.2.3", e);      
    }
  }
}
