...
public final class ScanRecord extends LongUpdatableRecord {
    ...
    protected int fill(PreparedStatement st, int idx) throws SQLException {
        ...
        setNullableString(idx++, st, filter);
        return idx;
    }
    ...
    protected int readAttributes(ResultSet set, int idx) throws SQLException {
        ...
        javaVersion = set.getString(idx++);
        javaVendor = set.getString(idx++);
        timestamp = set.getTimestamp(idx++);
        status = ScanStatus.valueOf(set.getString(idx++));
        partial = "Y".equals(set.getString(idx++));
        filter = set.getString(idx++);
        return idx;
    }
