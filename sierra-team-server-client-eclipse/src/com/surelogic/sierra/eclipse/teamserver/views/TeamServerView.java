...
public class TeamServerView extends ViewPart {
  ...
  public void createPartControl(Composite parent) {
    ...
    final Text logText = new Text(logGroup, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL);
    ...
    final MenuItem toggleLogVisibilityMenuItem = new MenuItem(logContextMenu, SWT.CHECK);
    ...
    final Action deleteServerDbAction = new Action("Wipe Local Team Server Database", IAction.AS_PUSH_BUTTON) {
      @Override
      public void run() {
        if (f_mediator != null) {
    ...
    final Action preferencesAction = new Action("Preferences...") {
      ...
      public void run() {
        PreferencesUtil.createPreferenceDialogOn(null,
            "com.surelogic.sierra.eclipse.teamserver.preferences.LocalTeamServerPreferencePage", null, null).open();
      }
    };
    menu.add(preferencesAction);

    f_mediator = new TeamServerMediator(command, status, portLabel, port, trafficLight, jettyConsoleLogItem, jettyRequestLogItem,
        logGroup, logText, toggleLogVisibilityMenuItem, showLogAction, deleteServerDbAction);
    f_mediator.init();
  }
