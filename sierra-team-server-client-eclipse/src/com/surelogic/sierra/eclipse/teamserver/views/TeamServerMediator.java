...
public final class TeamServerMediator implements ITeamServerObserver {
  ...
  private class LogSelectionListener implements Listener {
    ...
    public void handleEvent(Event event) {
      ...
      LocalTeamServerPreferencesUtility.setLogShowing(f_logShowingPersistenceNumber);
    }
  }

  private final ScheduledExecutorService f_executor = Executors.newScheduledThreadPool(2);

  TeamServerMediator(Button command, Link status, Label portLabel, Text port, Canvas trafficLight, ToolItem jettyConsoleLogItem,
      ToolItem jettyRequestLogItem, Group logGroup, Text logText, MenuItem toggleLogVisibilityMenuItem, Action showLogAction,
    ...
    f_teamServer = new TeamServer(LocalTeamServerPreferencesUtility.getPort(), f_executor);
  ...
  void init() {
    ...
    f_trafficLight.addPaintListener(new PaintListener() {
      ...
      public void paintControl(PaintEvent e) {
        ...
        if (e.widget instanceof Scrollable) {
          ...
          e.gc.drawImage(trafficLightImage, 0, 0, lightWidth, lightHeight, drawX, drawY, drawWidth, drawHeight);
        }
      }
    });
    ...
    f_jettyConsoleLogItem.addListener(SWT.Selection, new LogSelectionListener(f_jettyConsoleLog, 0));
    f_jettyRequestLogItem.addListener(SWT.Selection, new LogSelectionListener(f_jettyRequestLog, 1));

    /*
     * Which log is showing is persisted.
     */
    final int logShowing = LocalTeamServerPreferencesUtility.getLogShowing();
    if (logShowing == 0) {
    ...
    f_toggleLogVisibilityMenuItem.addListener(SWT.Selection, new Listener() {
      @Override
      public void handleEvent(Event event) {
        toggleLogVisibility();
      }
  ...
  /**
   * Refreshes the contents of this view. This method does not need to be called
   * from the SWT thread.
   */
  public void refresh() {
  ...
  void adjustControlState() {
    ...
    if (f_teamServer.isRunning()) {
      ...
      final String msg = I18N.msg("sierra.eclipse.teamserver.checking", f_teamServer.getPort());
      f_status.setText(msg);
  ...
  void toggleLogVisibility() {
    LocalTeamServerPreferencesUtility.setLogVisible(!LocalTeamServerPreferencesUtility.isLogVisible());
    adjustLogVisibility();
  }

  void deleteServerDb() {
    if (MessageDialog.openConfirm(f_command.getShell(), I18N.msg("sierra.eclipse.teamserver.confirmDataWipe.title"),
        I18N.msg("sierra.eclipse.teamserver.confirmDataWipe.msg"))) {
      FileUtility.recursiveDelete(LocalTeamServerPreferencesUtility.getSierraLocalTeamServerDirectory());
    }
  }

  void adjustLogVisibility() {
    final boolean visible = LocalTeamServerPreferencesUtility.isLogVisible();
  ...
  public void notifyStartupFailure(TeamServer server) {
     ...
     * We are not being called from the SWT thread.
     */
    startupFailureHelper(server.getProcessExitValue(), server.getProcessConsoleOutput());
  }

  /**
   * Puts up a dialog notifying the user that local team server startup failed.
   * This method does not need to be called from the SWT thread.
   */
  public void startupFailureHelper(final int exitValue, final String consoleMsg) {
    UIJob job = new SLUIJob() {
      @Override
      public IStatus runInUIThread(IProgressMonitor monitor) {
        final int errNo = 94;
  ...
  void openInBrowser(final URL url) {
  ...
  void updateLogText(final String text) {
