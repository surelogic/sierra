...
public final class TimeseriesPromptFromJob {
  ...
  final Set<String> f_timeseries;
  final Set<String> f_selectedTimeseries = new HashSet<>();
  final String f_projectName;
  final String f_serverLabel;
   ...
   */
  public TimeseriesPromptFromJob(Set<String> timeseries, String projectName, String serverLabel) {
    if (timeseries == null)
      throw new IllegalArgumentException(I18N.err(44, "timeseries"));
    f_timeseries = new HashSet<>(timeseries);
    if (projectName == null)
    ...
    final UIJob job = new SLUIJob() {
      ...
      public IStatus runInUIThread(IProgressMonitor monitor) {
        if (!f_timeseries.isEmpty()) {
          TimeseriesSelectionDialog dialog = new TimeseriesSelectionDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
              .getShell(), f_timeseries, f_projectName, f_serverLabel);
          if (dialog.open() != Window.CANCEL) {
            f_selectedTimeseries.addAll(dialog.getSelectedTimeseries());
  ...
  /**
   * Indicates that the resulting set of timeseries should be used for all other
   * projects sharing runs to the same Sierra server.
   * <p>
   * The results of this method are only valid after a call to {@link #open()}
   * when {@link #isCanceled()} is <code>false</code>.
   * 
   * @return <code>true</code> if the timeseries should be used for all other
   *         projects sharing runs to the same Sierra server, <code>false</code>
   *         otherwise.
   */
