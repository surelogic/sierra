...
public class NewScanAction extends AbstractProjectSelectedMenuAction {
  ...
  protected void runActionOn(final List<IJavaProject> selectedProjects) {
     ...
     */
    final SLStatus failed = SLLicenseUtility.validateSLJob(SLLicenseProduct.SIERRA, new NullSLProgressMonitor());
    if (failed != null) {
  ...
  protected JavaProjectSelectionDialog.Configuration getDialogInfo(List<IJavaProject> selectedProjects) {
    return new JavaProjectSelectionDialog.Configuration("Select project(s) to scan:", "Scan",
        SLImages.getImage(CommonImages.IMG_SIERRA_SCAN), selectedProjects,
        SierraPreferencesUtility.ALWAYS_ALLOW_USER_TO_SELECT_PROJECTS_TO_SCAN, SierraPreferencesUtility.LAST_TIME_PROJECTS_TO_SCAN);
  }
}
