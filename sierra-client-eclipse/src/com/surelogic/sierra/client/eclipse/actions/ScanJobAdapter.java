  ...
  public void done(IJobChangeEvent event) {
    ...
    if (event.getResult().equals(Status.OK_STATUS)) {
      ...
      if (EclipseUtility.getBooleanPreference(SierraPreferencesUtility.SHOW_BALLOON_NOTIFICATIONS))
        BalloonUtility.showMessage("Sierra " + scan + " completed on " + f_scanName, "You may now examine the results.");
      ...
      LOG.log(Level.SEVERE, "(top-level) Error while trying to run " + scan + " on " + f_scanName, t);
      if (event.getResult().isMultiStatus()) {
        for (IStatus s : event.getResult().getChildren()) {
          Throwable t1 = s.getException();
          LOG.log(Level.SEVERE, "(multi-status) Error while trying to run " + scan + " on " + f_scanName, t1);
        }
      }
    }
  }
