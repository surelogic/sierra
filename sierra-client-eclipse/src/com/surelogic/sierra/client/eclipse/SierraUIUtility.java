...
public final class SierraUIUtility {
     ...
     */
    @NonNull
    public static Image getImageForType(@Nullable String optionalProject,
            String packageName, String typeName) {
        IType jdtType = JDTUtility.findIType(optionalProject, packageName,
                typeName);
        if (jdtType == null) {
     ...
     * method or field declaration is opened. If nothing can be found the class
     * is opened.
     *
     * @param projectName
     *            a project name.
     ...
     */
    public static void tryToOpenInEditor(final String projectName,
            final String packageName, final String typeName,
            final int lineNumber, final long findingId) {
        if (lineNumber > 0) {
            if (JDTUIUtility.tryToOpenInEditor(projectName, packageName,
                    typeName, lineNumber)) {
                return;
            }
        }
        final Job job = new AbstractSierraDatabaseJob(
                "Querying details of finding " + findingId) {
            @Override
            protected IStatus run(final IProgressMonitor monitor) {
                monitor.beginTask("Looking up declaration information on "
                        + findingId, IProgressMonitor.UNKNOWN);
                try {
                    Data.getInstance().withReadOnly(new NullDBQuery() {
                        ...
                        NullResultHandler handler = new NullResultHandler() {
                            @Override
                        ...
                        public void doPerform(Query q) {
                            q.prepared("ViewSource.findIdentifiers", handler)
                                    .call(findingId);
                            if (!isField && !isMethod) {
                                q.prepared(
                                        "ViewSource.findIdentifiersBySecondarySources",
                                        handler).call(findingId);
                            }
                            final SLUIJob uiJob = new SLUIJob() {
                                ...
                                public IStatus runInUIThread(
                                    ...
                                    if (isField) {
                                        if (JDTUIUtility
                                                .tryToOpenInEditorUsingFieldName(
                                                        projectName,
                                    ...
                                    if (isMethod) {
                                        if (JDTUIUtility
                                                .tryToOpenInEditorUsingMethodName(
                                                        projectName,
                                                        packageName, typeName,
                                                        declName)) {
                                            return Status.OK_STATUS;
                                    ...
                                    // just open the type
                                    JDTUIUtility.tryToOpenInEditor(projectName,
                                            packageName, typeName);
    ...
    /**
     * Makes a best effort to lookup a more precise location in the class for a
     * particular artifact. Sierra uses this method to open the Java editor when
     * a finding id has 0 for the source line of code. If possible a method or
     * field declaration is opened. If nothing can be found the class is opened.
     *
     * @param projectName
     *            a project name.
     * @param packageName
    ...
    public static void tryToOpenInEditor(final String projectName,
        ...
        SourceDetail source = artifactDetail.getPrimarySource();
        String identifier = source.getIdentifier();
        if (identifier != null) {
            IdentifierType type = source.getIdentifierType();
            if (type == IdentifierType.FIELD) {
                JDTUIUtility.tryToOpenInEditorUsingFieldName(projectName,
                        packageName, typeName, identifier);
                return;
            } else if (type == IdentifierType.METHOD) {
                JDTUIUtility.tryToOpenInEditorUsingMethodName(projectName,
                        packageName, typeName, identifier);
                return;
            }
        }
        for (SourceDetail add : artifactDetail.getAdditionalSources()) {
            identifier = add.getIdentifier();
            if (identifier != null) {
                IdentifierType type = add.getIdentifierType();
                if (type == IdentifierType.FIELD) {
                    JDTUIUtility.tryToOpenInEditorUsingFieldName(projectName,
                            packageName, typeName, identifier);
                    return;
