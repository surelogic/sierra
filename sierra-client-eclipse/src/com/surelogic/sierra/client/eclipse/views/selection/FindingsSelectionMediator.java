...
public final class FindingsSelectionMediator implements IProjectsObserver, CascadingList.ICascadingListObserver,
    ISelectionManagerObserver, IFindingsObserver, IViewMediator {
  ...
  FindingsSelectionMediator(FindingsSelectionView view, Composite findingsPage, CascadingList cascadingList,
      ToolItem clearSelectionItem, Link breadcrumbs, Label findingsIcon, Link findingsStatus, ToolItem columnSelectionItem,
      ToolItem openSearchItem, ToolItem saveSearchesAsItem, ToolItem deleteSearchItem, Link savedSelections) {
    f_view = view;
  ...
  public void init() {
    ...
    for (final String name : Selection.TOGGLEABLE_COLUMN_TITLES) {
  ...
  private void clearToNewWorkingSelection() {
    ...
    f_first = new MRadioMenuColumn(f_cascadingList, f_workingSelection, null);
  ...
  private void openSelection(final Selection newSelection) {
    ...
    for (Filter filter : f_workingSelection.getFilters()) {
       ...
       */
      MFilterSelectionColumn fCol = new MFilterSelectionColumn(f_cascadingList, f_workingSelection, prevMenu, filter);
      fCol.init();
      /*
       * Create a menu
       */
      prevMenu = new MRadioMenuColumn(f_cascadingList, f_workingSelection, fCol);
      prevMenu.init();
    }
    if (f_workingSelection.isShowingFindings()) {
      prevMenu.setSelection("Show");
      MListOfFindingsColumn list = new MListOfFindingsColumn(f_cascadingList, f_workingSelection, prevMenu);
  ...
  private void updateColumnSelectionMenu(final Selection selection) {
    if (f_columnSelectionMenu != null && !f_columnSelectionMenu.isDisposed()) {
      for (MenuItem item : f_columnSelectionMenu.getItems()) {
        Column c = selection.getColumnByTitle(item.getText());
        if (c == null) {
  ...
  public void findingsLimited(boolean isLimited) {
    if (isLimited) {
      ...
      final int shouldBeShowing = f_workingSelection.getFindingCountPorous();
      final Image warning = SLImages.getImage(CommonImages.IMG_WARNING);
      f_findingsIcon.setImage(warning);
      f_findingsStatus.setText("<a href=\"preferences\">" + findingsListLimit + " of " + shouldBeShowing + " shown</a>");
    } else {
