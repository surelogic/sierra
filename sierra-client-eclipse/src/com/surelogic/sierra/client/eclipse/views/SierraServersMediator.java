...
public final class SierraServersMediator extends AbstractSierraViewMediator implements ISierraServerObserver, IProjectsObserver {
   ...
   * This should only be changed in the UI thread
   */
  private Map<NamedServer, List<ScanFilter>> localFilters = Collections.emptyMap();
   ...
   */
  private final Map<String, ProjectDO> projectMap = new HashMap<>();
  ...
  private class ServerActionListener extends ActionListener {
    ...
    ServerActionListener(final String text, final String tooltip, final String msg) {
      super(text, tooltip);
      msgIfNoServer = msg;
    }

    ServerActionListener(final Image image, final String tooltip, final String msg) {
      super(image, tooltip);
      msgIfNoServer = msg;
    }
    ...
    public final void run() {
      ...
      if (servers.size() == 1) {
        ...
        /*
         * } final ConnectedServer server = f_manager.getFocus(); if (server !=
         * null) { handleEventOnServer(server);
         */
      } else {
  ...
  private abstract class IJavaProjectsActionListener extends ServerActionListener {
    ...
    IJavaProjectsActionListener(final Image image, final String tooltip, final String msg) {
  ...
  public SierraServersMediator(final SierraServersView view, final TreeViewer statusTree) {
    ...
    f_toggleAutoSyncAction = new ServerActionListener("Toggle Auto-sync", "Toggle whether the server(s) automatically synchronize",
        "No server to toggle") {
      @Override
      protected void handleEventOnServer(final ConnectedServer server) {
    ...
    f_serverSyncAction = new ActionListener("Synchronize All", "Synchronize with all servers and connected projects") {
      @Override
      public ImageDescriptor getImageDescriptor() {
        return SLImages.getImageDescriptor(CommonImages.IMG_SIERRA_SYNC);
      }

      @Override
      public void run() {
        SierraServersAutoSync.asyncSyncWithServer(ServerSyncType.ALL, 0, ServerFailureReport.SHOW_DIALOG);
      }
    };
    ...
    f_openInBrowserAction = new ServerActionListener("Browse", "Open the selected team server in a Web browser",
        "No server to browse") {
      @Override
      protected void handleEventOnServer(final ConnectedServer server) {
    ...
    f_deleteServerAction = new IJavaProjectsActionListener(SLImages.getImage(CommonImages.IMG_EDIT_DELETE),
        "Deletes the selected team server location", "No server to delete") {
      @Override
      protected void run(final ConnectedServer server, final List<String> projectNames) {
        final String serverName = server.getName();
        final String msg;
        if (projectNames.isEmpty()) {
          msg = I18N.msg("sierra.eclipse.serverDeleteWarning", serverName);
        } else {
          msg = I18N.msg("sierra.eclipse.serverDeleteWarningConnected", serverName, serverName, serverName);
        }
    ...
    f_serverConnectAction = new ServerActionListener("No server to connect to, or no project to connect") {
      ...
      private void doConnect(final ConnectedServer server, final List<IJavaProject> projects) {
      ...
      protected void handleEventOnServer(final ConnectedServer server) {
        ...
        if (!projects.isEmpty()) {
          ...
          final MessageDialog dialog = new MessageDialog(f_statusTree.getTree().getShell(), "Synchronize Project(s) on Connect",
              null, "Do you want to synchronize the newly connected projects?", MessageDialog.QUESTION,
              new String[] { "Yes", "No" }, 0);
        ...
        final ConnectProjectsDialog dialog = new ConnectProjectsDialog(f_statusTree.getTree().getShell());
        dialog.open();
      }
    ...
    f_sendResultFiltersAction = new ServerActionListener("Send local scan filter pressed with no server focus.") {

      @Override
      protected void handleEventOnServer(final ConnectedServer server) {
        final String msg = "What do you want to call your scan filter " + "on the Sierra server '" + server.getName() + "'";
        final PromptForFilterNameDialog dialog = new PromptForFilterNameDialog(f_statusTree.getTree().getShell(), msg);
        if (dialog.open() == 0) {
          /*
           * Yes was selected, so send the local scan filters to the server.
           */
          if (SendScanFiltersJob.ENABLED) {
            final Job job = new SendScanFiltersJob(ServerFailureReport.SHOW_DIALOG, server, dialog.getText());
            job.schedule();
          }
        }
      }
    ...
    f_getResultFiltersAction = new ScanFilterActionListener("Overwrite local scan filter") {
      @Override
      protected void handleEventOnFilter(final ScanFilter f) {
        final String msg = "Do you want to overwrite your local scan filter with" + " the scan filter '" + f.getName() + "'?";
        final MessageDialog dialog = new MessageDialog(f_statusTree.getTree().getShell(), "Overwrite Local Scan Filter", null, msg,
            MessageDialog.QUESTION, new String[] { "Yes", "No" }, 0);
        if (dialog.open() == 0) {
          /*
           * Yes was selected, so get the result filters from the server.
           */
    ...
    f_serverPropertiesAction = new ServerActionListener("Edit server pressed with no server focus.") {
      @Override
      protected void handleEventOnServer(final ConnectedServer server) {
  ...
  public void init() {
    ...
    final Action sortByServerAction = new Action("Show by Team Server", IAction.AS_RADIO_BUTTON) {
      @Override
      public void run() {
        setSortByServer(isChecked());
      }
    };
    final Action sortByProjectAction = new Action("Show by Project", IAction.AS_RADIO_BUTTON) {
      @Override
      public void run() {
        setSortByServer(!isChecked());
      }
    ...
    f_statusTree.addDoubleClickListener(new IDoubleClickListener() {
      ...
      public void doubleClick(final DoubleClickEvent event) {
        ...
        final ServersViewContent item = (ServersViewContent) selection.getFirstElement();
        if (item.getData() instanceof FindingAudits) {
          ...
          if (info.isPartial()) {
            ...
            final ProjectStatus ps = (ProjectStatus) project.getData();
  ...
  void setupContextMenu(final Menu contextMenu) {
    ...
    final boolean enableSendFilters = SendScanFiltersJob.ENABLED && onlyBugLink && onlyServer;
    ...
    if (si.size() == 1) {
      final ServersViewContent c = (ServersViewContent) si.getFirstElement();
  ...
  private ConnectedServer inServer(ServersViewContent item) {
    while (item != null) {
      ...
      /*
       * if (data == null && SCAN_FILTERS.equals(item.getText())) { // Not a
       * server return null; }
       */
      item = item.getParent();
    }
  ...
  abstract class ProjectStatusCollector<T> {
    ...
    protected List<T> collectSelectedProjects(final IStructuredSelection si) {
      ...
      final List<T> projects = new ArrayList<>();
    ...
    private void collectProjects(final List<T> projects, final ServersViewContent parent, final boolean server) {
  ...
  void updateContents(final long now) throws Exception {
    ...
    try {
      ...
      final List<ProjectStatus> projects = new ArrayList<>();
        ...
        for (final IJavaProject jp : JDTUtility.getJavaProjects()) {
          ...
          if (!Projects.getInstance().contains(name)) {
            if (ConnectedServerManager.getInstance().isConnected(name)) {
              final String filterName = SettingQueries.scanFilterNameForProject(name).perform(q);
          ...
          final String filterName = SettingQueries.scanFilterNameForProject(name).perform(q);
          final ProjectStatus s = new ProjectStatus(jp, scan, info, numLocalAudits, numServerProblems, numProjectProblems, dbInfo,
              filterName);
          projects.add(s);
        }
  ...
  public void updateContentsInUI(final List<ProjectStatus> projects, final Map<NamedServer, List<ScanFilter>> filters) {
    ...
    final IStructuredSelection selection = (IStructuredSelection) f_statusTree.getSelection();
  ...
  static class TreeInput {
    ...
    private Map<Object, ServersViewContent> prepTranslation() {
      final Map<Object, ServersViewContent> map = new HashMap<>();
    ...
    private void processContent(final Map<Object, ServersViewContent> map, final ServersViewContent c) {
    ...
    ISelection translateSelection(final IStructuredSelection selection) {
      ...
      final List<ServersViewContent> selected = new ArrayList<>();
  ...
  private ServersViewContent[] createServerItems() {
    final List<ServersViewContent> content = new ArrayList<>();
  ...
  private void createUnknownServers(final List<ServersViewContent> content) {
    for (final Map.Entry<NamedServer, List<ScanFilter>> e : localFilters.entrySet()) {
      final NamedServer label = e.getKey();
      final ConnectedServer server = f_manager.getServerByUuid(label.getUuid());
      if (server != null) {
      ...
      final ServersViewContent serverRoot = new ServersViewContent(null, SLImages.getImage(CommonImages.IMG_SIERRA_SERVER_GRAY));
      final List<ServersViewContent> children = new ArrayList<>();
      for (final ScanFilter f : e.getValue()) {
        final String name = f.getName();
        final ServersViewContent filter = createLabel(serverRoot, children, name, ChangeStatus.NONE);
        filter.setData(f);
      }
  ...
  private ServersViewContent createServerItem(final ConnectedServer server) {
    final List<ServersViewContent> serverContent = new ArrayList<>();

    final ServersViewContent serverNode = new ServersViewContent(null, SLImages.getImage(CommonImages.IMG_SIERRA_SERVER));
    serverNode.setData(server);

    final ServersViewContent scanFilters = createScanFilters(serverNode, server);
    if (scanFilters != null) {
      serverContent.add(scanFilters);
    }
    if (!f_manager.getProjectsConnectedTo(server).isEmpty()) {
      final ServersViewContent projects = new ServersViewContent(serverNode, SLImages.getImage(CommonImages.IMG_PROJECT));
    ...
    serverNode.setText(status3.getLabel() + server.getName() + " [" + server.getLocation().createHomeURL() + ']');
    return serverNode;
  }

  private ServersViewContent createScanFilters(final ServersViewContent serverNode, final ConnectedServer server) {
    final List<ScanFilter> filters = localFilters.get(new NamedServer(server.getName(), server.getUuid()));
    if (!showFiltersOnServer || filters == null || filters.isEmpty()) {
      return null;
    }
    final ServersViewContent root = new ServersViewContent(serverNode, SLImages.getImage(CommonImages.IMG_FILTER));
    ...
    for (final ScanFilter f : filters) {
      ...
      if (showFiltersOnServer) {
        filter = createLabel(filterRoot, children, f.getName(), ChangeStatus.NONE);
        filter.setData(f);
      }
    }
  ...
  private void createUnassociatedProjectItems(final List<ServersViewContent> content) {
    ...
    for (final ProjectStatus ps : projects) {
      ...
      if (server == null) {
        ...
        final ServersViewContent project = new ServersViewContent(parent, SLImages.getImage(CommonImages.IMG_PROJECT));
  ...
  private void createLocalScanFilterItems(final List<ServersViewContent> content) {
    final ServersViewContent filterRoot = new ServersViewContent(null, SLImages.getImage(CommonImages.IMG_FILTER));
    ...
    for (final Map.Entry<NamedServer, List<ScanFilter>> e : localFilters.entrySet()) {
      ...
      for (final ScanFilter f : e.getValue()) {
        ...
        final ServersViewContent filter = createLabel(filterRoot, children, name, ChangeStatus.NONE);
        filter.setData(f);
      }
    }
  ...
  private void createProjectItems(final ServersViewContent parent, final ConnectedServer server) {
    final List<ServersViewContent> content = new ArrayList<>();

    for (final String projectName : f_manager.getProjectsConnectedTo(server)) {
      ...
      if (s == null) {
        ...
        if (jp != null) {
          // No scan data?
          final ServersViewContent root = createProjectItem(parent, server, projectName);
          ...
          if ((p != null) && p.exists()) {
            ...
            final ServersViewContent root = createProjectItem(parent, server, projectName);
            content.add(root);
            createLabel(root, "Project no longer in workspace ... no info available");
            continue;
            // The project probably once existed. Ignore it now.
          }
        }
      }
      final ServersViewContent root = new ServersViewContent(parent, SLImages.getImage(CommonImages.IMG_PROJECT));
  ...
  private ServersViewContent createLabel(final ServersViewContent parent, final String text) {
    return createLabel(parent, text, ChangeStatus.NONE);
  }

  private ServersViewContent createLabel(final ServersViewContent parent, final String text, final ChangeStatus delta) {
  ...
  private ServersViewContent createProjectItem(final ServersViewContent parent, final ConnectedServer server,
  ...
  private void initProjectItem(final ServersViewContent root, final ConnectedServer server, final ProjectStatus ps) {
    final List<ServersViewContent> contents = new ArrayList<>();
    ...
    if ((ps.scanDoc != null) && ps.scanDoc.exists()) {
      ...
      /*
       * } else { ServersViewContent scan = new ServersViewContent(root, null);
       * scan.setText("Needs a local scan"); contents.add(scan);
       */
    }

    if (ps.numLocalAudits > 0) {
      final ServersViewContent audits = new ServersViewContent(root, SLImages.getImage(CommonImages.IMG_SIERRA_STAMP));
      contents.add(audits);

      final List<ServersViewContent> auditContents = new ArrayList<>();
    ...
    if (server != null && ps.numProjectProblems > 0) {
      final ServersViewContent problems = new ServersViewContent(root, SLImages.getImage(CommonImages.IMG_WARNING));
      contents.add(problems);
      problems.setText(ps.numProjectProblems + " consecutive failure" + s(ps.numProjectProblems) + " getting server info from "
          + server.getName());
      problems.setServerStatus(ServerStatus.WARNING);
    }
    /*
     * if (ps.localDBInfo != null) { final ServersViewContent scanFilter = new
     * ServersViewContent(root, SLImages.getImage(CommonImages.IMG_FILTER));
    ...
    if (ps.filterName != null) {
      final ServersViewContent scanFilter = new ServersViewContent(root, SLImages.getImage(CommonImages.IMG_FILTER));
  ...
  private ServersViewContent createAuditItems(final ServersViewContent audits, final List<ServersViewContent> contents,
      final int numAudits) {
  ...
  private ServersViewContent[] createProjectItems() {
    final ServersViewContent[] content = new ServersViewContent[projects.size()];
    int i = 0;
    for (final ProjectStatus ps : projects) {
      final ConnectedServer server = f_manager.getServer(ps.name);
      content[i] = new ServersViewContent(null, SLImages.getImage(CommonImages.IMG_PROJECT));
  ...
  void addServerMenuItems(final Menu contextMenu, final AutoSyncType syncType, final boolean enableConnect,
    ...
    if (enableConnect) {
      final MenuItem serverConnectItem = AbstractSierraView.createMenuItem(contextMenu, "Connect Projects...",
    ...
    final MenuItem serverPropertiesItem = new MenuItem(contextMenu, SWT.PUSH);
  ...
  void addSendLocalFilter_MenuItem(final Menu contextMenu) {
    final MenuItem sendResultFilters = AbstractSierraView.createMenuItem(contextMenu, "Send Local Scan Filter As...",
        CommonImages.IMG_FILTER);
    sendResultFilters.addListener(SWT.Selection, f_sendResultFiltersAction);
  }

  void addProjectMenuItems(final Menu contextMenu, final boolean allHasScans, final boolean allConnected) {
    if (!allConnected) {
      final MenuItem serverConnectItem = AbstractSierraView.createMenuItem(contextMenu, "Connect Projects...",
          CommonImages.IMG_SIERRA_SERVER);
      serverConnectItem.addListener(SWT.Selection, f_serverConnectAction);
    ...
    final MenuItem scanProjectItem = AbstractSierraView.createMenuItem(contextMenu, "Scan Project", CommonImages.IMG_SIERRA_SCAN);
    final MenuItem rescanProjectItem = AbstractSierraView.createMenuItem(contextMenu, "Re-Scan Changes in Project",
        CommonImages.IMG_SIERRA_SCAN_DELTA);
    if (allConnected) {
      final MenuItem synchProjects = AbstractSierraView.createMenuItem(contextMenu, "Synchronize Project",
    ...
    if (allConnected) {
      ...
      if (allHasScans) {
        final MenuItem publishScansItem = AbstractSierraView.createMenuItem(contextMenu, "Publish Latest Scan",
            CommonImages.IMG_SIERRA_PUBLISH);
        publishScansItem.addListener(SWT.Selection, f_publishScansAction);
      }
      final MenuItem disconnectProjectItem = AbstractSierraView.createMenuItem(contextMenu, "Disconnect",
  ...
  void addScanFilterMenuItems(final Menu contextMenu) {
    final MenuItem getResultFilters = AbstractSierraView.createMenuItem(contextMenu, "Copy to Local Scan Filter", (Image) null);
  ...
  class ContentProvider implements ITreeContentProvider {
    ...
    public Object[] getChildren(final Object parentElement) {
      if (parentElement instanceof ServersViewContent) {
        final ServersViewContent[] children = ((ServersViewContent) parentElement).getChildren();
    ...
    public boolean hasChildren(final Object element) {
      if (element instanceof ServersViewContent) {
        final ServersViewContent[] children = ((ServersViewContent) element).getChildren();
