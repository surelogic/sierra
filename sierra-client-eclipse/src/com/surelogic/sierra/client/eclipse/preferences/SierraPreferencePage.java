...
public class SierraPreferencePage extends AbstractCommonPreferencePage {
  ...
  public SierraPreferencePage() {
    super("sierra.eclipse.", SierraPreferencesUtility.getSwitchPreferences());
  }
  ...
  protected Control createContents(final Composite parent) {
    ...
    dataGroup.setText(I18N.msg("sierra.eclipse.preference.page.group.data"));
    dataGroup.setLayout(new GridLayout());

    final Label dataDirectory = new Label(dataGroup, SWT.NONE);
    dataDirectory.setText(SierraPreferencesUtility.getSierraDataDirectory().getAbsolutePath());
    ...
    f_findingsListLimit = new IntegerFieldEditor(SierraPreferencesUtility.FINDINGS_LIST_CUTOFF,
        I18N.msg("sierra.eclipse.preference.page.findingsListLimit"), diGroup);
    f_findingsListLimit.fillIntoGrid(diGroup, 2);
    f_findingsListLimit.setPage(this);
    f_findingsListLimit.setPreferenceStore(EclipseUIUtility.getPreferences());
    f_findingsListLimit.load();

    f_balloonFlag = new BooleanFieldEditor(SierraPreferencesUtility.SHOW_BALLOON_NOTIFICATIONS,
    ...
    f_selectProjectsToScan = new BooleanFieldEditor(SierraPreferencesUtility.ALWAYS_ALLOW_USER_TO_SELECT_PROJECTS_TO_SCAN,
        I18N.msg("sierra.eclipse.preference.page.selectProjectsToScan"), diGroup);
    f_selectProjectsToScan.fillIntoGrid(diGroup, 2);
    f_selectProjectsToScan.setPage(this);
    f_selectProjectsToScan.setPreferenceStore(EclipseUIUtility.getPreferences());
    f_selectProjectsToScan.load();

    f_showMarkersInJavaEditorFlag = new BooleanFieldEditor(SierraPreferencesUtility.SHOW_MARKERS,
        I18N.msg("sierra.eclipse.preference.page.showMarkersInJavaEditorFlag"), diGroup);
    f_showMarkersInJavaEditorFlag.fillIntoGrid(diGroup, 2);
    f_showMarkersInJavaEditorFlag.setPage(this);
    f_showMarkersInJavaEditorFlag.setPreferenceStore(EclipseUIUtility.getPreferences());
    f_showMarkersInJavaEditorFlag.load();

    f_showAbove = new RadioGroupFieldEditor(SierraPreferencesUtility.SHOW_MARKERS_AT_OR_ABOVE_IMPORTANCE,
        I18N.msg("sierra.eclipse.preference.page.showAbove"), 1, new String[][] {
            { Importance.CRITICAL.toStringSentenceCase(), Importance.CRITICAL.toString() },
            { Importance.HIGH.toStringSentenceCase(), Importance.HIGH.toString() },
            { Importance.MEDIUM.toStringSentenceCase(), Importance.MEDIUM.toString() },
            { Importance.LOW.toStringSentenceCase(), Importance.LOW.toString() },
            { Importance.IRRELEVANT.toStringSentenceCase(), Importance.IRRELEVANT.toString() } },
    ...
    memoryGroup.setText(I18N.msg("sierra.eclipse.preference.page.group.scan"));

    final int estimatedMax = MemoryUtility.computeMaxMemorySizeInMb();
    int mb = EclipseUtility.getIntPreference(SierraPreferencesUtility.TOOL_MEMORY_MB);
    if (mb > estimatedMax) {
      mb = estimatedMax;
      EclipseUtility.setIntPreference(SierraPreferencesUtility.TOOL_MEMORY_MB, mb);
    }

    final String label = I18N.msg(TOOL_MB_LABEL, SLUtility.toStringHumanWithCommas(mb));
    f_toolMemoryMB = new ScaleFieldEditor(SierraPreferencesUtility.TOOL_MEMORY_MB, label + "     ", memoryGroup);
    ...
    f_toolMemoryMB.load();
    f_toolMemoryMB.getScaleControl().addListener(SWT.Selection, new Listener() {
      @Override
    ...
    f_estimate.setText(I18N.msg("sierra.eclipse.preference.page.computedMaxToolMemoryLabel",
        SLUtility.toStringHumanWithCommas(estimatedMax)));

    f_saveResources = new BooleanFieldEditor(SierraPreferencesUtility.ALWAYS_SAVE_RESOURCES,
        I18N.msg("sierra.eclipse.preference.page.saveModified"), memoryGroup);
    ...
    if (XUtil.useExperimental) {
      ...
      exportButton.addListener(SWT.Selection, new Listener() {
        @Override
        public void handleEvent(final Event event) {
          new ExportQueryDialog(EclipseUIUtility.getShell(), SierraDataSource.getManager()).open();
        }
      });
    }
    return panel;
  ...
  void updateMBInLabel() {
    ...
    f_toolMemoryMB.setLabelText(I18N.msg(TOOL_MB_LABEL, SLUtility.toStringHumanWithCommas(mb)));
  }
