...
public class ScanDataPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
  ...
  protected Control createContents(final Composite parent) {
    ...
    deleteDatabase.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true, false));
  ...
  private static class Mediator {
    ...
    final Table f_projectTable;
    final Button f_deleteSelectedProjects;
    final Button f_deleteDatabase;

    Mediator(final Table projectTable, final Button deleteSelectedProjects, final Button deleteDatabase) {
    ...
    void init() {
      ...
      f_projectTable.addListener(SWT.Selection, new Listener() {
        ...
        public void handleEvent(final Event event) {
          f_deleteSelectedProjects.setEnabled(f_projectTable.getSelectionCount() != 0);
        }
      });
      ...
      f_deleteSelectedProjects.addListener(SWT.Selection, new Listener() {
        ...
        public void handleEvent(final Event event) {
          ...
          DeleteProjectDataJob.utility(projectNames, f_projectTable.getShell(), false);
        }
      });
      ...
      f_deleteDatabase.addListener(SWT.Selection, new Listener() {
        ...
        public void handleEvent(final Event event) {
          ...
          if (!MessageDialog.openConfirm(f_projectTable.getShell(), "Confirm Sierra Data Deletion", b.toString())) {
            return; // bail
          }
    ...
    void setTableContents(final List<String> projectNames) {
      if (!f_projectTable.isDisposed()) {
        ...
        for (final String projectName : projectNames) {
          final TableItem item = new TableItem(f_projectTable, SWT.NULL);
