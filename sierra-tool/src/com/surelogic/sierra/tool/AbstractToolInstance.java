...
public abstract class AbstractToolInstance extends AbstractSLJob implements IToolInstance {
  ...
  /**
   * Gets the compilation unit name from a Java source file name e.g. Baz from
   * /foo/bar/Baz.java
   */
  ...
  private final List<IToolTarget> srcTargets = new ArrayList<>();
  private final List<IToolTarget> binTargets = new ArrayList<>();
  private final List<IToolTarget> auxTargets = new ArrayList<>();
  ...
  protected AbstractToolInstance(IToolFactory factory, Config config, ILazyArtifactGenerator gen, boolean close) {
    ...
    genHandle = gen;
    closeWhenDone = close;
    debug = SLLogger.getLogger().isLoggable(Level.FINE);
  }
  ...
  /**
   * Encapsulates most of the details of using ILazyArtifactGenerator,
   * ArtifactGenerator, besides the actual creation of artifacts/errors
   */
  ...
  public interface TargetPrep {
    /**
     * @param f
     *          the Java file location
     */
    void prep(File f) throws Exception;
  }

  /**
   * Iterates through all the Java source files, calling TargetPrep.prep() for
   * each one
   */
  ...
  /**
   * Iterates through all the Java binary files, calling TargetPrep.prep() for
   * each one
   */
  ...
  /**
   * Iterates through all the Java libraries, calling TargetPrep.prep() for each
   * one
   */
  ...
  /**
   * A helper class that encapsulates the source directories that any given Java
   * source file was found under. Mostly used to compute the package names for
   * each file
   * 
   * @author edwin
   */
  public static class SourceRoots {
    ...
    public void initSourceInfo(final SourceInfo info, final String fileName) {
      ...
      if (separator < 0) {
        // Default package
        info.packageName = "";
        info.cuName = file.substring(0, file.length() - JAVA_SUFFIX_LEN);
      } else {
        info.packageName = file.substring(0, separator).replace(File.separatorChar, '.');
        info.cuName = file.substring(separator + 1, file.length() - JAVA_SUFFIX_LEN);
      }
    }
  }

  /**
   * Iterates through all the Java source files, calling SourcePrep.prep() for
   * each one
   * 
   ...
   */
  protected static void setSourceLocation(SourceLocationBuilder sourceLocation, SourceInfo info) {
