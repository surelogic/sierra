...
public class RunSierra extends Task {
  ...
  private Config createConfig() {
    ...
    System.setProperty(ToolUtil.TOOLS_PATH_PROP_NAME, libHome.getAbsolutePath());
    ...
    config.putPluginDir(AbstractLocalSLJob.COMMON_PLUGIN_ID, new File(libHome, "common.jar").getAbsolutePath());
    config.putPluginDir(SierraToolConstants.MESSAGE_PLUGIN_ID, new File(libHome, "sierra-message.jar").getAbsolutePath());
    config.putPluginDir(SierraToolConstants.PMD_PLUGIN_ID, new File(toolsDir, "pmd").getAbsolutePath());
    config.putPluginDir(SierraToolConstants.FB_PLUGIN_ID, new File(toolsDir, "findbugs").getAbsolutePath());
    config.putPluginDir(SierraToolConstants.TOOL_PLUGIN_ID, new File(libHome, "sierra-tool.jar").getAbsolutePath());
    // System.out.println("Using source level "+scan.getSource());
    config.setSourceLevel(sourceLevel);

    config.setScanDocument(new File(outputDir, name + SierraToolConstants.PARSED_FILE_SUFFIX).getAbsoluteFile());
    config.setLogPath(new File(outputDir, name + AbstractRemoteSLJob.LOG_SUFFIX).getAbsolutePath());
    return config;
  }
  ...
  protected String[] loadProperties(Config cfg) {
    ...
    if (propFile != null) {
      final Properties props = SureLogicToolsPropertiesUtility.readFileOrNull(getProperties());
      ...
      if (props != null) {
        log(String.format("Loading properties file at %s.", getProperties()));
